...
FITXER_SORTIDA = 'verbs_anotats_num.json'
...
def anotar_verbs():
    ...
    for verb in tqdm(llista_verbs, desc="Processant verbs", unit="verb"):
        ...
        try:
            ...
            # Busquem TOTS els resultats de la cerca, confiant en l'algoritme del DIEC
            elements_resultat = soup_cerca.find_all('a', class_='resultAnchor')
            
            if not elements_resultat:
                resultats[verb] = {"categories": [], "estat": "No trobat al diccionari"}
                continue
                
            totes_les_troballes = []
            
            # Iterem sobre cadascun dels resultats obtinguts (haver1, haver2... o queixar-se)
            for element in elements_resultat:
                ...
                troballes_actuals = [m.group(0).strip().replace('\n', ' ') for m in patro_categories.finditer(text_pagina)]
                totes_les_troballes.extend(troballes_actuals)
                
                time.sleep(0.2)
            
            # Unifiquem resultats, netegem espais i eliminem duplicats
            totes_les_troballes = [re.sub(r'\s+', ' ', t) for t in totes_les_troballes] 
            categories_trobades = sorted(list(set(totes_les_troballes)))
